...
import {
  ...
  imagePreview,
  resourceFilter,
} from "@/components/ActionsSheet/sheets";
...
export default function App() {
  ...
  const [filterField, setFilterField] = useState<Array<FilterFieldType>>([
    "audio",
    "image",
    "video",
  ]);
      ...
      headerRight: () => (
        <Pressable
          onPress={async () => {
            ...
            setFilterField(
              sheetValue?.filterType ? [...sheetValue.filterType] : [],
            );
  ...
  function Item({ value }: { value: Resource }) {
    ...
    return (
      <>
        ...
        >
          <NativeView
            style={[{ opacity: resource.status ? 1 : 0.5 }, styles.item]}
          >
